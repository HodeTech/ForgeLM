...
def _add_audit_subcommand(subparsers) -> None:
    ...
    p.add_argument(
        ...
        help=(
            ...
            "PII_SEVERITY. Cards must pass a real issuer prefix AND the Luhn checksum, and IBANs "
            "the ISO 7064 mod-97 checksum, so a hit is indistinguishable from a genuine value "
            "(the issuer-prefix requirement excludes IMEIs and order numbers that a bare Luhn "
            "check would flag — every IMEI passes Luhn by construction). "
            "Government IDs, emails and phone numbers are reported but NEVER gate; the tier "
            "decides, not the detector, so a checksum-validated TC national ID reports without "
            "failing because it sits below critical. "
            ...
            "Note the flag is corpus-wide, not per-category: suppressing a card false positive also "
            "stops a genuine IBAN from gating, so prefer --pii-mask when only the cards are noise. "
