...
**Yayınlandı:** `v0.10.0` — "Verilen Sözler" — PyPI 2026-07-20.  Bu projenin vaat edip teslim etmediği her maddeyi kapatır; yedi adım, her birinde bir Opus ve bir Sonnet review turu.  Bulguların üçü güvenlikle ilgiliydi ve hiçbiri asıl listede yoktu: yanlış yapılandırılmış bir üretken sınıflandırıcı güvenlik kapısından **sahte PASS** döndürebiliyordu, `forgelm audit`'in kimlik-bilgisi sızıntı kapısı **hiç ateşlenmemişti** (artık `3` döner — içinde secret olan korpusları denetleyen pipeline'lar için kırıcı bir değişiklik), ve yok edilmiş Annex IV kanıtı temiz sertifikalanabiliyordu.  Ayrıca `EXIT_INTEGRITY_FAILURE = 6`, HF revision pinning, `python -m forgelm`, `forgelm/safety/` paket bölünmesi ve `forgelm/verify.py` ile gelir.  Altı yeni CI guard'ı; 3374 → 4460 test.  Bkz. [releases.md](roadmap/releases.md#v0100--promises-kept-2026-07-20).
