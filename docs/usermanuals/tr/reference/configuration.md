...
lora:
  ...
  use_dora: false                             # method: "dora" için deprecated boolean kısayolu; v0.9.0'da kaldırılır
  use_rslora: false                           # method: "rslora" için deprecated boolean kısayolu; v0.9.0'da kaldırılır
```

`LoraConfigModel`'de `modules_to_save` veya `use_pissa` alanı yoktur — `extra="forbid"` ikisini de `--dry-run`'da reddeder. PiSSA initialisation bir boolean toggle değil `method: "pissa"` ile seçilir; `use_dora` / `use_rslora`, `method: "dora"` / `method: "rslora"` için deprecated boolean kısayollardır, v0.9.0'da kaldırılması planlanmıştır — ikisini birden ayarlamak, veya birini çelişen açık bir `method:` ile ayarlamak config hatasıdır.
