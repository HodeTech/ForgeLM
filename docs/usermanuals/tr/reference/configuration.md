...
`revision`, 40-hex bir Hub commit SHA'sını (gerçekten sabitleyen tek değer) veya `main` gibi bir branch/tag'i kabul eder — bir ref kabul edilir ama ForgeLM uyarır, çünkü upstream onu başka yere işaretleyebilir. Tokenizer'ı, VLM processor'ını, ağırlıkları ve `--fit-check` config problamasını aynı commit'e sabitler ve çözülen SHA Annex IV paketine kaydedilir. Yerel bir dizine karşı ayarlandığında uyarır ve hiçbir şey yapmaz: diskteki bir yolun Hub commit'i yoktur. Dört kardeş pin alanı vardır ve dördü de **uygulanıyor**: `synthetic.teacher_revision`, `evaluation.llm_judge.judge_model_revision`, `training.grpo_reward_model_revision` ve `evaluation.safety.classifier_revision` — her biri bir commit SHA'sına çözülür ve bu SHA, o depo için her `from_pretrained` çağrısına — tokenizer ve model dâhil — geçilir, böylece ikisi asla farklı commit'lerden gelemez. Hiçbir SHA teyit edilemediğinde literal yine aynen geçilir ve hiçbir şeyin doğrulanmadığını söyleyen bir `WARNING` yazılır. `classifier_revision` bu sürüme kadar hiçbir yükleyiciye ulaşmıyordu — auto-revert kapısının arkasındaki zarar sınıflandırıcısı config'ten bağımsız olarak hareketli bir varsayılan daldan yükleniyordu — ve yalnızca eğitim zamanı güvenlik kapısı için geçerlidir: bağımsız `forgelm safety-eval` hiçbir config almaz ve sınıflandırıcısını sabitlenmemiş yükler. Dört pin de artık bir uyum artefaktına ulaşır: Annex IV paketi, değişmemiş `model_lineage.base_model_revision`'ın yanında `model_lineage.component_revisions` taşır.
...
lora:
  ...
  use_dora: false                             # method: "dora" için deprecated boolean kısayolu; v1.0.0'da kaldırılması planlanıyor
  use_rslora: false                           # method: "rslora" için deprecated boolean kısayolu; v1.0.0'da kaldırılması planlanıyor
```
