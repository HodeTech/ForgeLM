...
:::warn
**Eğitimde değil, eval'de OOM.** Eval genelde kayar-pencere veya packing olmadan koşar — eğitim sığsa bile OOM verebilir. Ayrı bir `evaluation.max_length` alanı yoktur; `model.max_length` hem eğitim hem eval geçişini yönetir (TRL her ikisi için de aynı trainer config'ini kullanır) — tek kolunuz `model.max_length`'in kendisini düşürmektir, bu da eğitim context penceresini küçültür.
:::
