...
- **GGUF export sonrası CI/CD yayın kapılarında.** Her `forgelm export` adımından sonra çalıştırın ve **sıfırdan farklı herhangi bir çıkış kodunda yayını başarısız sayın** (kamuya açık sözleşme `0/1/2/3/4` setini takip eder; manuel içindeki [Çıkış Kodları](#/reference/exit-codes) referansına bakın. `1` en sık kapı tripi'dir — çağıran/girdi + bütünlük uyuşmazlığı — ancak `2` mevcut dosyalardaki gerçek I/O hatalarını kapsar, üst pipeline'lardan diğer sıfırdan farklı kodlar da iletilebilir).
