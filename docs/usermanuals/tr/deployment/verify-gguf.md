...
sequenceDiagram
    ...
    opt gguf paketi yüklü
        ...
        alt okuyucu istisna fırlatır
            Verify-->>CI: çıkış 6 (magic OK ama meta veri bozuk — bütünlük arızası)
        end
    end
    opt sidecar mevcut
...
| **Meta veri bloğu** | İsteğe bağlı `gguf` Python paketi yüklüyse. | Yazıcı yarıda çöktü ya da dosya kesildi — tensor tablosu artık kendi içinde tutarlı değil. Dosya *gerçekten* bir GGUF (magic geçti), dolayısıyla bu bir bütünlük arızasıdır. | `6` |
...
| `GGUF metadata block could not be parsed: struct.error: …` | Dosya kesilmiş ya da yazıcı export ortasında çökmüş. | `6` |
...
| `1` | Magic uyuşmazlığı (hiç GGUF değil), bozuk sidecar, VEYA regular file olmayan bir path (operatör yanlış argüman geçirdi). Hiçbir şey karşılaştırılmadı. |
| `2` | Gerçek bir dosyayı okurken I/O hatası (permission denied, okuma sırasında disk hatası). |
| `6` | Bütünlük arızası: dosya *gerçekten* bir GGUF (magic OK) ve ya meta veri bloğu ayrıştırılamadı ya da SHA-256 sidecar'ı uyuşmuyor. |
