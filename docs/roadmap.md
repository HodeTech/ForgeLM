...
**Released:** `v0.10.0` — "Promises Kept" — PyPI 2026-07-20.  Closes every item this project had promised and not delivered, across seven steps with an Opus and a Sonnet review round each.  Three of the findings were security-relevant and none was on the original list: a misconfigured generative classifier could return a **false PASS** from the safety gate, `forgelm audit`'s credential-leak gate had **never fired** (it now exits `3` — a breaking change for pipelines auditing corpora that contain secrets), and destroyed Annex IV evidence could be certified clean.  Also ships `EXIT_INTEGRITY_FAILURE = 6`, HF revision pinning, `python -m forgelm`, the `forgelm/safety/` package split and `forgelm/verify.py`.  Six new CI guards; 3374 → 4460 tests.  See [releases.md](roadmap/releases.md#v0100--promises-kept-2026-07-20).
