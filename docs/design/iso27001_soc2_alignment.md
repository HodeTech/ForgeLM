...
| A.5.19 Information security in supplier relationships | FL-helps | `_fingerprint_hf_revision` records the **dataset** repo's Hub commit SHA, graded `loaded` (the SHA `load_dataset` was pinned to), `unverified` (a lookup not tied to any load), `local_path` (files on disk — no Hub commit exists) or `unresolved` (no SHA obtained; the reason is stated); `model.revision` pins the **base model** and `model_lineage.base_model_revision` records what the load used; SBOM lists every transitive dep. The LLM judge and GRPO reward model are now pinned by `evaluation.llm_judge.judge_model_revision` and `training.grpo_reward_model_revision`. **Residual risk:** neither of those pins is recorded in any artefact, and the safety classifier (`classifier_revision` reaches no loader) and merge sources are still loaded unpinned — see `docs/reference/configuration.md` "Hub revision pinning" | Vendor risk assessments for HF, base-model providers | Vendor questionnaires deployer-side |
