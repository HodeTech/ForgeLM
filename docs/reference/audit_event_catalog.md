...
| `pipeline.stage_completed` | A pipeline stage finished — `gate_decision=passed` on success, `failed` on a non-revert failure. | `pipeline_run_id`, `stage_name`, `gate_decision`, `metrics`, `auto_revert_triggered` | 12      |
