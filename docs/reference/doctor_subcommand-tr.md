...
| `extras.<ad>` | İçe aktarılabiliyorsa `pass`, yoksa kurulum ipucuyla `warn` | Her opsiyonel extra için bir satır: `qlora`, `unsloth`, `distributed`, `eval`, `tracking`, `tracking-mlflow`, `export`, `ingestion`, `ingestion-pii-ml`, `ingestion-scale`. `merging` extra'sının satırı bilerek **yoktur** — model birleştirme çekirdek bağımlılıklarla native olarak uygulandığından o extra bir no-op'tur. |
